...
describe('/redirect', () => {
  it('GET redirected to https://github.com/rakatar/juice-shop when this URL is passed as "to" parameter', () => {
    return frisby.get(`${URL}/redirect?to=https://github.com/rakatar/juice-shop`, { redirect: 'manual' })
      .expect('status', 302)
  })
  ...
  it('GET redirected to target URL in "to" parameter when a allow-listed URL is part of the query string', () => {
    return frisby.get(`${URL}/redirect?to=/score-board?satisfyIndexOf=https://github.com/rakatar/juice-shop`)
      .expect('status', 200)
      .expect('header', 'content-type', /text\/html/)
